...
\begin{algorithmic}
\State Initialize $v_0^{0}(x) = 1$, $v_1^{0}(x) = 1$
\For{$k = 0, 1, 2, \dots$}
    \State $v_0^{k+1}(x) = \frac{\mu_0(x)}{\int_{\R^d} K_\epsilon(x, y) v_1^k(y) \, dy}$
    \State $v_1^{k+1}(y) = \frac{\mu_1(y)}{\int_{\R^d} K_\epsilon(x, y) v_0^{k+1}(x) \, dx}$
\EndFor
\State $\pi(x, y) = v_0^{k}(x) K_\epsilon(x, y) v_1^{k}(y)$
